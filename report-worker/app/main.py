...
if config.GCP_PROJECT_ID:
    try:
        # This function would need to be defined or imported
        # setup_cloud_logging()
...
def handle_pubsub():
    ...
    if isinstance(pubsub_message, dict) and "data" in pubsub_message:
        try:
            ...
            if data_str:  # Check if data is not empty
                ...
                logger.info(
                    "Received Pub/Sub message with empty data field. Assuming default job type."
                )
    ...
    logger.info(
        f"Determined trigger date (in {config.MARKET_CLOSE_TIMEZONE}): {trigger_date.isoformat()}"
    )
