...
class CommissionMailer:
    ...
    def send_pending_reports(self, db: Session) -> Dict[str, Any]:
        ...
        pending_records = db.query(CommissionMonthly).filter(
            and_(
        ...
        for record in pending_records:
            try:
                ...
                logger.error(f"Failed to send report for {record.follower_id}: {str(e)}")
                results['failed'] += 1
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt < self.max_retries - 1:
                    logger.warning(f"Attempt {attempt + 1} failed for {record.follower_email}: {str(e)}")
    ...
    def _generate_email_html(self, record: CommissionMonthly, month_name: str, excel_url: str) -> str:
