...
def mock_pnl_daily_records():
    ...
    for date, pnl in zip(dates, pnl_values):
        record = MagicMock()
        record.date = date
...
class TestReportGenerator:
    ...
    async def test_generate_excel_report_success(
        ...
        with patch('report_worker.app.service.report_generator.generate_excel_report') as mock_excel:
...
async def test_generate_follower_reports_convenience_function(mock_settings, sample_follower):
    ...
    with patch('report_worker.app.service.report_generator.ReportGenerator') as mock_generator_class:
