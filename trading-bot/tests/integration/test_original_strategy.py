...
from ib_insync import Stock, Contract, Order, BarData

from spreadpilot_core.ibkr.client import IBKRClient
from spreadpilot_core.models.alert import Alert
from trading_bot.app.config import Settings, ORIGINAL_EMA_STRATEGY
...
def mock_ibkr_client(mock_ibkr_server):
    ...
    client.get_contract_details = AsyncMock(side_effect=lambda symbol, **kwargs: [MagicMock(contract=mock_ibkr_server.get_contract(symbol))])
    client.get_historical_data = AsyncMock(side_effect=lambda contract, **kwargs: mock_ibkr_server.get_historical_data(contract.symbol, kwargs.get('duration_str', '1 D'), kwargs.get('bar_size_setting', '5 mins')))
    client.place_order = AsyncMock(side_effect=lambda contract, order: mock_ibkr_server.place_order(contract, order))
...
async def trading_service(mock_settings, mock_sheets_client):
    ...
    with patch('trading_bot.app.service.base.firebase_admin.initialize_app'):
        with patch('trading_bot.app.service.base.firestore.client'):
            with patch('trading_bot.app.service.base.secretmanager.SecretManagerServiceClient'):
...
async def test_original_strategy_initialization(trading_service, mock_ibkr_client):
    ...
    with patch('trading_bot.app.service.original_strategy_handler.IBKRClient', return_value=mock_ibkr_client):
        ...
        assert mock_ibkr_client.get_historical_data.call_count == len(ORIGINAL_EMA_STRATEGY["symbols"])
...
async def test_trading_service_run_with_strategy(trading_service, mock_ibkr_client):
    ...
    with patch('trading_bot.app.service.original_strategy_handler.IBKRClient', return_value=mock_ibkr_client):
        ...
        with patch.object(trading_service.original_strategy_handler, 'run', AsyncMock()) as mock_run:
...
async def test_trading_service_shutdown_with_strategy(trading_service, mock_ibkr_client):
    ...
    with patch('trading_bot.app.service.original_strategy_handler.IBKRClient', return_value=mock_ibkr_client):
        ...
        with patch.object(trading_service.original_strategy_handler, 'shutdown', AsyncMock()) as mock_shutdown:
...
async def test_strategy_interaction_with_ibkr(trading_service, mock_ibkr_client, mock_ibkr_server):
    ...
    with patch('trading_bot.app.service.original_strategy_handler.IBKRClient', return_value=mock_ibkr_client):
        ...
        with patch.object(trading_service.original_strategy_handler, '_check_bullish_crossover', return_value=True):
            with patch.object(trading_service.original_strategy_handler, '_check_bearish_crossover', return_value=False):
                # Process the bar
    ...
    with patch('trading_bot.app.service.original_strategy_handler.IBKRClient', return_value=mock_ibkr_client):
        ...
        bars = [
            ...
            BarData(
                ...
                volume=1000, average=20.2, barCount=100
            ),
            # Bearish crossover for SOXL
            BarData(
                date=datetime.datetime(2025, 4, 18, 10, 5, 0),
                open=20.5, high=21.5, low=20.0, close=21.0,
                volume=1200, average=20.8, barCount=120
            ),
            # No crossover
            BarData(
                date=datetime.datetime(2025, 4, 18, 10, 10, 0),
                open=21.0, high=22.0, low=20.5, close=21.5,
                volume=1300, average=21.2, barCount=130
            )
        ]
        ...
        with patch.object(trading_service.original_strategy_handler, '_check_bullish_crossover', return_value=False):
            with patch.object(trading_service.original_strategy_handler, '_check_bearish_crossover', return_value=True):
        ...
        assert mock_ibkr_client.place_order.call_count >= 4  # At least 4 orders (2 for SOXS, 2 for SOXL)
