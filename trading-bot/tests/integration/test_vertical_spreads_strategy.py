...
from trading_bot.app.config import Settings, VERTICAL_SPREADS_STRATEGY
...
async def test_vertical_spreads_strategy_initialization():
    ...
    with patch('trading_bot.app.service.base.connect_to_mongo', return_value=mongo_client), \
        ...
        service.vertical_spreads_strategy_handler.ibkr_client.is_connected = AsyncMock(return_value=True)
        service.vertical_spreads_strategy_handler.ibkr_client.get_positions = AsyncMock(return_value={})
...
async def test_vertical_spreads_strategy_signal_processing():
    ...
    with patch('trading_bot.app.service.base.connect_to_mongo', return_value=mongo_client), \
         patch('trading_bot.app.service.base.get_mongo_db', return_value=mongo_db), \
         patch('trading_bot.app.service.vertical_spreads_strategy_handler.get_ny_time') as mock_get_ny_time:
        ...
        service.vertical_spreads_strategy_handler.ibkr_client.is_connected = AsyncMock(return_value=True)
        service.vertical_spreads_strategy_handler.ibkr_client.get_positions = AsyncMock(return_value={})
        service.vertical_spreads_strategy_handler.ibkr_client.check_margin_for_trade = AsyncMock(return_value=(True, None))
        service.vertical_spreads_strategy_handler.ibkr_client.place_vertical_spread = AsyncMock(return_value={
            "status": "FILLED",
            "trade_id": "123456",
            "fill_price": 0.75,
...
async def test_vertical_spreads_strategy_time_value_monitoring():
    ...
    with patch('trading_bot.app.service.base.connect_to_mongo', return_value=mongo_client), \
        ...
        service.vertical_spreads_strategy_handler.ibkr_client.connect = AsyncMock(return_value=True)
        service.vertical_spreads_strategy_handler.ibkr_client.is_connected = AsyncMock(return_value=True)
        service.vertical_spreads_strategy_handler.ibkr_client.get_positions = AsyncMock(return_value={"QQQ-380-C": 1})
        
        # Mock the time value calculation to return a value below the threshold
        service.vertical_spreads_strategy_handler._calculate_time_value = AsyncMock(return_value=0.05)
        ...
        service.vertical_spreads_strategy_handler._calculate_time_value.assert_called_once_with("QQQ-380-C", 1)
        service.vertical_spreads_strategy_handler._close_position.assert_called_once_with("QQQ-380-C", 1)