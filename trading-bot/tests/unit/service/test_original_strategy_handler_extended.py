...
from ib_insync import Contract, Order, BarData, Stock, Position
...
data = {
    'open': [20.0, 20.0, 20.5, 21.0, 21.5, 22.0, 22.5, 23.0, 23.5, 24.0, 24.5, 25.0, 25.5, 26.0, 26.5, 27.0, 27.5, 28.0, 28.5, 29.0, 29.5, 30.0],
    'high': [21.0, 21.0, 21.5, 22.0, 22.5, 23.0, 23.5, 24.0, 24.5, 25.0, 25.5, 26.0, 26.5, 27.0, 27.5, 28.0, 28.5, 29.0, 29.5, 30.0, 30.5, 31.0],
    'low': [19.0, 19.0, 20.0, 20.5, 21.0, 21.5, 22.0, 22.5, 23.0, 23.5, 24.0, 24.5, 25.0, 25.5, 26.0, 26.5, 27.0, 27.5, 28.0, 28.5, 29.0, 29.5],
    'close': [20.0, 20.5, 21.0, 21.5, 22.0, 22.5, 23.0, 23.5, 24.0, 24.5, 25.0, 25.5, 26.0, 26.5, 27.0, 27.5, 28.0, 28.5, 29.0, 29.5, 30.0, 30.5],
    'volume': [1000, 1100, 1200, 1300, 1400, 1500, 1600, 1700, 1800, 1900, 2000, 2100, 2200, 2300, 2400, 2500, 2600, 2700, 2800, 2900, 3000, 3100]
}
...
async def strategy_handler(mock_trading_service, mock_ibkr_client):
    ...
    with patch('app.service.original_strategy_handler.IBKRClient', return_value=mock_ibkr_client):
...
async def test_fetch_initial_positions_success(strategy_handler, mock_ibkr_client):
    ...
    mock_ibkr_client.get_positions.return_value = [mock_position1, mock_position2, mock_position3]
...
async def test_process_bar_insufficient_data(strategy_handler, mock_ibkr_client):
    ...
    mock_df = MagicMock()
    mock_df.set_index.return_value = pd.DataFrame({'open': 20.0, 'high': 21.0, 'low': 19.0, 'close': 20.5, 'volume': 1000},
                                                 index=[datetime.datetime(2025, 4, 18, 10, 0, 0)])
...
async def test_process_bar_ema_calculation_error(strategy_handler, mock_ibkr_client):
    ...
    with patch('pandas.DataFrame', return_value=mock_df), \
         patch('pandas.concat', return_value=concat_result), \
         patch.object(strategy_handler, '_calculate_ema', side_effect=Exception("EMA calculation error")):
        # Call the method
        await strategy_handler._process_bar(symbol, bar)
...
async def test_send_alert_error(strategy_handler):
    ...
    with patch('app.service.original_strategy_handler.Alert') as MockAlert:
        ...
        strategy_handler.service.alert_manager.create_alert.side_effect = Exception("Alert creation error")
...
async def test_process_eod_contract_not_found(strategy_handler, mock_ibkr_client):
    ...
    mock_ibkr_client.get_contract_details.side_effect = [[], [MagicMock(contract=MOCK_STOCK_CONTRACT)]]
...
async def test_process_eod_order_placement_error(strategy_handler, mock_ibkr_client):
    ...
    mock_ibkr_client.place_order.side_effect = [Exception("Order placement error"), MagicMock()]
