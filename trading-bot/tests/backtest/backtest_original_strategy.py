...
class BacktestEngine:
    ...
    def process_bar(self, timestamp, symbol, open_price, high_price, low_price, close_price, volume):
        ...
        if self.config["trailing_stop_pct"] > 0:
            ...
            if current_position > 0:  # Long position
                ...
                if low_price <= stop_price:
                    ...
                    logger.info(f"Signal: {timestamp} TRAILING_STOP for {symbol} long @ ${stop_price:.2f}")
                ...
                if high_price >= stop_price:
                    ...
                    logger.info(f"Signal: {timestamp} TRAILING_STOP for {symbol} short @ ${stop_price:.2f}")
        ...
        for timestamp, group in all_data.groupby(level=0):
            ...
            if current_date != date:
                if current_date is not None and not eod_processed:
                    ...
                    eod_prices = {
                        symbol: self.historical_data[symbol].loc[
                            self.historical_data[symbol].index.date == current_date
                        ]["close"].iloc[-1]
