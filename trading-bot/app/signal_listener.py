...
class SignalListener:
    ...
    def __init__(
        ...
        self.gsheet_client: Optional[gspread.Client] = None
    ...
    async def stop(self):
        ...
        if self.redis_client:
            await self.redis_client.aclose() if hasattr(self.redis_client, 'aclose') else self.redis_client.close()
    ...
    async def _poll_for_todays_signal(self) -> Optional[Signal]:
        ...
        for attempt in range(1, self.max_poll_attempts + 1):
            try:
                ...
                if attempt % 10 == 0:  # Log every 10 attempts (50 seconds)
                    logger.info(f"Polling attempt {attempt}/{self.max_poll_attempts} - no signal yet")
    ...
    def _find_signal_for_date(self, all_values: list, target_date: date) -> Optional[Signal]:
    ...
    async def fetch_signal_now(self) -> Optional[Signal]:
