...
class PnLService:
    ...
    async def _calculate_follower_mtm(self, follower_id: str):
        ...
        try:
            ...
            position_doc = await positions_collection.find_one(
                {"follower_id": follower_id, "date": today}
            )

            if not position_doc:
                logger.debug(
                    f"No positions found for follower {follower_id} on {today}"
                )
                return
            ...
            if ibkr_pnl is None:
                logger.warning(
                    f"Could not get P&L from IBKR for follower {follower_id}"
                )
                return

            # Calculate additional metrics
            position_count = self._count_active_positions(position_doc)
            total_market_value = await self._calculate_market_value(
                client, position_doc
            )
            ...
            logger.debug(
                f"Stored MTM P&L for follower {follower_id}: "
    ...
    async def _perform_daily_rollup(self):
        ...
        try:
            ...
            logger.info(
                f"Completed daily P&L rollup for {len(self.active_followers)} followers"
            )
    ...
    async def _rollup_daily_pnl(self, follower_id: str, trading_date: date):
        ...
        try:
            async with get_postgres_session() as session:
                ...
                max_profit = max(
                    (s.total_pnl for s in intraday_snapshots), default=Decimal("0")
                )
                max_drawdown = min(
                    (s.total_pnl for s in intraday_snapshots), default=Decimal("0")
                )
    ...
    async def _calculate_monthly_commission(
        ...
        try:
            ...
            commission_pct = (
                Decimal(str(follower_data["commission_pct"])) / 100
    ...
    async def get_real_time_pnl(self, follower_id: str) -> dict | None:
    ...
    async def mark_commission_paid(
        self, commission_id: str, payment_reference: str
    ) -> bool:
        ...
        try:
            async with get_postgres_session() as session:
                result = await session.execute(
                    select(CommissionMonthly).where(
                        CommissionMonthly.id == commission_id
                    )
                )
                ...
                if commission:
                    ...
                    logger.info(
                        f"Marked commission {commission_id} as paid with reference {payment_reference}"
                    )
                    return True
                else:
