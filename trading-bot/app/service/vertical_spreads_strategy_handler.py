...
class VerticalSpreadsStrategyHandler:
    ...
    async def run(self, shutdown_event: asyncio.Event):
        ...
        try:
            while not shutdown_event.is_set():
                ...
                if is_signal_time and (self._last_signal_check is None or 
                                      (ny_time - self._last_signal_check).total_seconds() > 60):
                ...
                if (self._last_time_value_check is None or 
                    (ny_time - self._last_time_value_check).total_seconds() > 60):  # Check every minute
    ...
    async def _process_signal(self, signal: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(f"Processing signal: {strategy} {qty_per_leg} contracts, strikes: {strike_long}/{strike_short}")
            ...
            for key, qty in positions.items():
                if "QQQ" in key and qty != 0:
                    ...
                    if time_value is not None:
                        ...
                        if time_value < 0.10:
                            logger.info(f"Time Value below threshold for {key}: ${time_value:.2f}, closing position")
                            await self._close_position(key, qty)
        
        except Exception as e:
            logger.error(f"Error monitoring Time Value: {e}", exc_info=True)

    async def _calculate_time_value(self, position_key: str, qty: float) -> Optional[float]:
        """
    ...
    async def _close_position(self, position_key: str, qty: float):
        ...
        try:
            ...
            if trade:
                logger.info(f"Closed position {position_key} with market order: {trade.order.orderId}")
