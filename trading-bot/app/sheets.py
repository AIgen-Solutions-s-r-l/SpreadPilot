...
class GoogleSheetsClient:
    ...
    def __init__(self, sheet_url: str, api_key: str | None = None):
    ...
    async def fetch_signal(self) -> dict[str, str | int | float] | None:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(url) as response:
                    ...
                    if (
                        qty_index is None
                        or buy_put_index is None
                        or sell_put_index is None
                        or sell_call_index is None
                        or buy_call_index is None
                    ):
                        logger.warning(
                    ...
                    if (
                        len(row) <= strategy_index
                        or len(row) <= qty_index
                        or len(row) <= strike_long_index
                        or len(row) <= strike_short_index
                    ):
                        logger.warning(
