...
from spreadpilot_core.models.pnl import (
    Trade, Quote, PnLIntraday, PnLDaily, PnLMonthly, CommissionMonthly
)
...
class TestPnLService:
    ...
    @pytest.mark.asyncio
    async def test_calculate_mtm_with_positions(self, pnl_service, mock_db_session, random_positions):
        ...
        with patch('spreadpilot_core.pnl.service.get_postgres_session') as mock_get_session:
            ...
            mock_db_session.execute.return_value.scalars.return_value.all.return_value = []  # No trades
    ...
    @pytest.mark.asyncio
    async def test_monthly_rollup_with_commission(self, pnl_service, mock_db_session, mock_follower_data):
    ...
    @pytest.mark.asyncio
    async def test_commission_calculation_positive_pnl(self, pnl_service, mock_db_session, mock_follower_data):
    ...
    @pytest.mark.asyncio
    async def test_commission_calculation_negative_pnl(self, pnl_service, mock_db_session, mock_follower_data):
    ...
    @pytest.mark.asyncio
    async def test_commission_calculation_zero_pnl(self, pnl_service, mock_db_session, mock_follower_data):
    ...
    @pytest.mark.asyncio
    async def test_update_existing_commission(self, pnl_service, mock_db_session, mock_follower_data):
    ...
    async def test_quote_subscription_loop(self, pnl_service, random_positions):
        ...
        pnl_service.set_callbacks(
            get_positions_fn=mock_get_positions,
            subscribe_tick_fn=mock_subscribe_tick
        )
