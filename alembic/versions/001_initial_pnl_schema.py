...
def upgrade() -> None:
    ...
    op.create_index('ix_trades_follower_time', 'trades', ['follower_id', 'trade_time'], unique=False)
    ...
    op.create_index('ix_quotes_contract_time', 'quotes', ['symbol', 'contract_type', 'strike', 'expiration', 'quote_time'], unique=False)
    op.create_index('ix_quotes_time', 'quotes', ['quote_time'], unique=False)

    # Create pnl_intraday table
    op.create_table('pnl_intraday',
        sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('follower_id', sa.String(length=50), nullable=False),
    ...
    op.create_index('ix_pnl_intraday_follower_time', 'pnl_intraday', ['follower_id', 'snapshot_time'], unique=False)
    ...
    op.create_index('ix_pnl_daily_follower_date', 'pnl_daily', ['follower_id', 'trading_date'], unique=False)
    op.create_index('ix_pnl_daily_date', 'pnl_daily', ['trading_date'], unique=False)

    # Create pnl_monthly table
    op.create_table('pnl_monthly',
        sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('follower_id', sa.String(length=50), nullable=False),
    ...
    op.create_index('ix_pnl_monthly_follower_period', 'pnl_monthly', ['follower_id', 'year', 'month'], unique=False)
