...
async def test_pnl_month_with_params(async_client: AsyncClient, auth_headers: dict):
    ...
    response = await async_client.get("/api/v1/pnl/month?year=2025&month=1", headers=auth_headers)
    assert response.status_code in [status.HTTP_200_OK, status.HTTP_404_NOT_FOUND]

    # Test with invalid month
    response = await async_client.get("/api/v1/pnl/month?year=2025&month=13", headers=auth_headers)
...
async def test_logs_with_filters(async_client: AsyncClient, auth_headers: dict):
    ...
    # Test with limit
    response = await async_client.get("/api/v1/logs/?limit=10", headers=auth_headers)
    assert response.status_code == status.HTTP_200_OK
    logs = response.json()["logs"]
    assert len(logs) <= 10

    # Test with service filter
    response = await async_client.get("/api/v1/logs/?service=trading-bot", headers=auth_headers)
    assert response.status_code == status.HTTP_200_OK

    # Test with level filter
    response = await async_client.get("/api/v1/logs/?level=ERROR", headers=auth_headers)
    assert response.status_code == status.HTTP_200_OK

    # Test with search
    response = await async_client.get("/api/v1/logs/?search=error", headers=auth_headers)
    assert response.status_code == status.HTTP_200_OK
