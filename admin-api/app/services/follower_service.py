...
from app.schemas.follower import FollowerCreate, FollowerUpdate, FollowerResponse
...
class FollowerService:
    ...
    async def get_followers(self, skip: int = 0, limit: int = 100, status: Optional[str] = None) -> List[FollowerResponse]:
        ...
        # Execute query
        cursor = collection.find(query).skip(skip).limit(limit).sort("created_at", pymongo.DESCENDING)
    ...
    async def get_follower(self, follower_id: str) -> Optional[FollowerResponse]:
    ...
    async def get_follower_by_email(self, email: str) -> Optional[FollowerResponse]:
    ...
    async def update_follower(self, follower_id: str, follower_update: FollowerUpdate) -> Optional[FollowerResponse]:
        """Update a follower."""
        collection = await self.get_collection()
