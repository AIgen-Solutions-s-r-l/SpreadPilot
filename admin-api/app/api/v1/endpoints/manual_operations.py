...
class ManualCloseRequest(BaseModel):
    ...
    position_ids: Optional[list[str]] = Field(default=None, description="Specific position IDs to close (if close_all is False)")
    reason: Optional[str] = Field(default="Manual close requested by admin", description="Reason for manual close")
...
@router.post("/manual-close", response_model=ManualCloseResponse, dependencies=[Depends(get_current_user)])
async def manual_close_positions(request: ManualCloseRequest = Body(...)):
    ...
    try:
        ...
        logger.info(f"Manual close requested for follower {request.follower_id}, operation_id: {operation_id}")
