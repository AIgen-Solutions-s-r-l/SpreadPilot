...
async def test_vault_secret_retrieval(test_mongo_db: AsyncIOMotorDatabase):
    ...
    try:
        ...
        with patch(
            "spreadpilot_core.utils.vault.get_vault_client",
...
@pytest.mark.asyncio
async def test_vault_integration_with_gateway_manager(
    ...
    follower = Follower(
        id=follower_id,
        email="gateway-vault-test@example.com",
        iban="IBAN-GW-VAULT",
        ibkr_username="gw_vault_user",
...
async def test_minio_report_storage(test_mongo_db: AsyncIOMotorDatabase):
    ...
    mock_minio_client.presigned_get_object.return_value = (
        "https://minio.example.com/bucket/report.pdf?signature=xyz"
    )
...
async def test_vault_secret_rotation():
    ...
    new_credentials = {"IB_USER": "original_user", "IB_PASS": "new_rotated_pass_2024"}
