...
from spreadpilot_core.ibkr.gateway_manager import GatewayManager, GatewayStatus, GatewayInstance
...
class MockContainer:
    ...
    def __init__(self, container_id: str, name: str, status: str = "running"):
        ...
        self.status = status
        self.attrs = {
            'Created': time.time() - 10  # Container created 10 seconds ago
        }
        self._removed = False
...
async def gateway_manager(mock_docker_client):
    ...
    with patch('spreadpilot_core.ibkr.gateway_manager.docker.from_env', return_value=mock_docker_client):
...
async def test_get_client_returns_connected_ib_instance(gateway_manager, mock_db, capsys):
    ...
    with patch('spreadpilot_core.ibkr.gateway_manager.get_mongo_db', return_value=mock_db):
        with patch('spreadpilot_core.ibkr.gateway_manager.IB', return_value=MockIBClient(should_echo=True)):
            await gateway_manager.start()
