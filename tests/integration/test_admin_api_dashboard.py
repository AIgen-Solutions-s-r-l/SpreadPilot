...
import admin_api.app.api.v1.endpoints.dashboard as dashboard_module # Import dashboard module for direct patching
...
from tests.integration.conftest import admin_api_client, admin_api_test_client, test_mongo_db
...
async def test_dashboard_api_endpoints( # Ensure async def
    ...
    try:
        follower_data_active = {
            "_id": str(ObjectId()), "email": "dash-active@example.com", "iban": "IBAN-DASH-A",
            "ibkr_username": "dashuserA", "ibkr_secret_ref": "secretA", "commission_pct": 10,
            "enabled": True, "state": FollowerState.ACTIVE.value, "daily_pnl": 100.50, "monthly_pnl": 500.75
        }
        follower_data_inactive = {
            "_id": str(ObjectId()), "email": "dash-inactive@example.com", "iban": "IBAN-DASH-I",
            "ibkr_username": "dashuserI", "ibkr_secret_ref": "secretI", "commission_pct": 15,
            "enabled": False, "state": FollowerState.DISABLED.value, "daily_pnl": -20.0, "monthly_pnl": -100.0
        }
...
async def test_dashboard_api_error_handling( # Ensure async def
    ...
    with patch("admin_api.app.services.follower_service.FollowerService.get_followers", new_callable=AsyncMock) as mock_get_followers_summary:
        ...
        with patch("admin_api.app.api.v1.endpoints.dashboard.get_settings", return_value=MagicMock()):
    ...
    with patch("admin_api.app.services.follower_service.FollowerService.get_followers", new_callable=AsyncMock) as mock_get_followers_summary_alt:
...
async def test_periodic_follower_update_task(
    ...
    mock_followers = [
        ...
        MagicMock(id=follower_id_error, enabled=True, spec=Follower), # Enabled but will cause error
    ]

    # Patch the service methods used by the task
    # Patch the service method and broadcast function used by the task
    with patch("admin_api.app.services.follower_service.FollowerService.get_followers", new_callable=AsyncMock) as mock_get_all, \
         patch("admin_api.app.api.v1.endpoints.dashboard.broadcast_updates", new_callable=AsyncMock) as mock_broadcast:
        ...
        task = asyncio.create_task(
            periodic_follower_update_task(
                follower_service=service,
                interval_seconds=test_interval
            )
        )
        await asyncio.sleep(test_interval * 2) # Allow task to run at least once based on the test interval
...
async def test_websocket_dashboard_connect(
    ...
    follower_data = {
        "email": "websocket-test@example.com", "iban": "IBAN-WEBSOCKET-TEST",
        "ibkr_username": "websocket_test_user", "commission_pct": 15, "ibkr_secret_ref": "test-secret-ref"
    }
...
async def test_websocket_dashboard_multiple_clients(
    ...
    follower_data = {
        "email": "multi-client-test@example.com", "iban": "IBAN-MULTI-CLIENT-TEST",
        "ibkr_username": "multi_client_user", "commission_pct": 10, "ibkr_secret_ref": "test-secret-ref"
    }
    response = admin_api_test_client.post("/api/v1/followers", json=follower_data)
    assert response.status_code == 201, f"Failed to create follower: {response.text}"
    created_follower = response.json()
