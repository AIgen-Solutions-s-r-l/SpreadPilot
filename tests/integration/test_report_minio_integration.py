...
async def test_monthly_report_minio_storage(test_mongo_db: AsyncIOMotorDatabase):
    ...
    mock_minio_client.presigned_get_object.return_value = f"https://minio.example.com/reports/test-report.pdf?X-Amz-Signature=test"
    ...
    with patch('report_worker.app.service.generator.generate_pdf_report', return_value=io.BytesIO(pdf_content)):
        ...
        stored_report = await test_mongo_db.monthly_reports.find_one({"report_id": report_data["report_id"]})
...
async def test_report_retrieval_from_minio(test_mongo_db: AsyncIOMotorDatabase):
    ...
    mock_minio_client.get_object.return_value = MagicMock(
        read=MagicMock(return_value=mock_pdf_content),
        close=MagicMock()
    )
