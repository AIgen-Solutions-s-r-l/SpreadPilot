...
async def test_toggle_follower(
    ...
    try:
        ...
        with patch("admin_api.app.api.v1.endpoints.followers.get_settings", return_value=MagicMock()):
            response_toggle_back = await admin_api_client.post(f"/api/v1/followers/{follower_id_str}/toggle")
...
def test_create_follower_validation_error(admin_api_test_client_sync: TestClient):
    ...
    invalid_follower_data = {
        "email": "invalid-email", "iban": "NL91ABNA0417164306",
        "ibkr_username": "validation-user", "ibkr_secret_ref": "secret", "commission_pct": 10,
    }
    ...
    missing_field_data = {
        "email": "valid@example.com", "ibkr_username": "validation-user2",
        "ibkr_secret_ref": "secret2", "commission_pct": 12,
    }
    ...
    invalid_commission_data = {
        "email": "valid2@example.com", "iban": "NL91ABNA0417164307",
        "ibkr_username": "validation-user3", "ibkr_secret_ref": "secret3", "commission_pct": 110,
    }
    ...
    assert "Input should be less than or equal to 100" in response.text or "commission_pct" in response.text
...
async def test_followers_api_error_handling(
    ...
    follower_data = {
        "email": "duplicate-test@example.com", "iban": "IBAN-DUP-1",
        "ibkr_username": "dupuser1", "ibkr_secret_ref": "secretdup1", "commission_pct": 10
    }
    ...
    try:
        follower_data_dup = {
            "email": "duplicate-test@example.com", "iban": "IBAN-DUP-2",
            "ibkr_username": "dupuser2", "ibkr_secret_ref": "secretdup2", "commission_pct": 15
        }
        ...
        with patch("admin_api.app.services.follower_service.FollowerService.toggle_follower_enabled", new_callable=AsyncMock) as mock_toggle:
            mock_toggle.side_effect = Exception("Toggle Service Error")
            response_toggle_err = await admin_api_client.post(f"/api/v1/followers/{created_id}/toggle")
        ...
        with patch("admin_api.app.services.follower_service.FollowerService.trigger_close_positions", new_callable=AsyncMock) as mock_trigger_err:
...
async def test_list_followers_error_handling(
    ...
    with patch("admin_api.app.services.follower_service.FollowerService.get_followers", new_callable=AsyncMock) as mock_get_followers:
        ...
        assert "Internal Server Error" in response.text or "Failed to retrieve followers" in response.text
...
async def test_create_follower_service_error(
    ...
    follower_data = {
        "email": "create-error@example.com", "iban": "IBAN-CERR",
        "ibkr_username": "cerruser", "ibkr_secret_ref": "secretcerr", "commission_pct": 10
    }
    with patch("admin_api.app.services.follower_service.FollowerService.create_follower", new_callable=AsyncMock) as mock_create:
...
async def test_toggle_follower_service_error(
    ...
    follower_data = {
        "_id": str(ObjectId()), "email": "toggle-error@example.com", "iban": "IBAN-TERR",
        "ibkr_username": "terruser", "ibkr_secret_ref": "secretterr", "commission_pct": 10
    }
