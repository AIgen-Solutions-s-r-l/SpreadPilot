...
from spreadpilot_core.models import Follower, Position, Trade, Alert
...
def mock_google_sheets():
    ...
    with patch('spreadpilot_core.signal_listener.SignalListener') as mock:
        ...
        mock_instance.get_latest_signals = AsyncMock(return_value=[
            {
...
async def test_error_handling_workflow(
    ...
    alert = await db.alerts.find_one({
        "type": "SIGNAL_ERROR",
        "reference_id": invalid_signal["signal_id"]
    })
