...
class TestGatewayManager:
    ...
    def setup_method(self):
        ...
        with patch('spreadpilot_core.ibkr.gateway_manager.docker.from_env'):
            self.gateway_manager = GatewayManager(
                healthcheck_interval=1,  # Short interval for testing
                max_startup_time=2
            )
    ...
    async def test_start_gateway_allocates_resources(self):
        ...
        self.gateway_manager.docker_client.containers.get.side_effect = docker.errors.NotFound("Not found")
        
        follower = MockFollower(id="test_follower", ibkr_username="test_user")
        
        # Act
        with patch.object(self.gateway_manager, '_get_ibkr_credentials_from_vault', return_value=None):
    ...
    async def test_stop_follower_gateway_force_removal_on_error(self):
        ...
        mock_container.remove.side_effect = [Exception("Remove failed"), None]  # Fail first, succeed on force
    ...
    async def test_vault_credentials_retrieval(self):
        ...
        vault_credentials = {
            "IB_USER": "vault_username",
            "IB_PASS": "vault_password"
        }
        ...
        # Act
        with patch.object(self.gateway_manager, '_get_ibkr_credentials_from_vault', return_value=vault_credentials):
    ...
    async def test_reconnect_on_disconnected_client(self):
        ...
        with patch.object(self.gateway_manager, '_connect_ib_client', return_value=AsyncMock()) as mock_connect:
            # Act
            await self.gateway_manager._check_gateway_health(gateway, time.time())
    ...
    async def test_get_client_creates_new_connection_if_needed(self):
        ...
        with patch.object(self.gateway_manager, '_connect_ib_client', return_value=mock_new_client) as mock_connect:
            # Act
            client = await self.gateway_manager.get_client("test_follower")
    ...
    async def test_reload_followers_adds_new_followers(self):
        ...
        with patch('spreadpilot_core.ibkr.gateway_manager.get_mongo_db', return_value=mock_db):
            with patch.object(self.gateway_manager, '_start_gateway', return_value=AsyncMock()) as mock_start:
                # Act
    ...
    async def test_reload_followers_removes_inactive_followers(self):
        ...
        with patch('spreadpilot_core.ibkr.gateway_manager.get_mongo_db', return_value=mock_db):
            with patch.object(self.gateway_manager, '_stop_gateway', return_value=AsyncMock()) as mock_stop:
                # Act
                await self.gateway_manager.reload_followers()
