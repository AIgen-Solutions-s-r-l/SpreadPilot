...
class TestEmailWithMinIO:
    ...
    @patch('report_worker.app.service.notifier_minio.send_email')
    @patch('report_worker.app.service.notifier_minio.minio_service')
    def test_email_with_attachments_fallback(self, mock_minio, mock_send_email, test_follower, temp_files):
    ...
    @patch('report_worker.app.service.notifier_minio.send_email')
    @patch('report_worker.app.service.notifier_minio.minio_service')
    def test_email_with_minio_upload_failure(self, mock_minio, mock_send_email, test_follower, temp_files):
...
class TestSMTPEmailSender:
    ...
    def test_smtp_uri_parsing(self):
        ...
        sender = SMTPEmailSender(
            smtp_uri="smtp://user123:pass456@mail.example.com:465",
            from_email="sender@example.com"
        )
...
class TestReportServiceEnhanced:
    ...
    async def test_process_monthly_reports(self, mock_pnl, mock_generator, mock_email, mock_mongo):
        ...
        mock_db.__getitem__.return_value.find.return_value.__aiter__ = AsyncMock(return_value=iter([follower_doc]))
