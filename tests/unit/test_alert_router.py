...
class TestAlertRouter:
    ...
    async def test_redis_stream_subscription(self, alert_router, fake_redis, mock_alert):
        ...
        with patch.object(
            alert_router, "_send_telegram_with_retry", new_callable=AsyncMock
        ) as mock_telegram:
            with patch.object(
                alert_router, "_send_email_with_retry", new_callable=AsyncMock
            ) as mock_email:
                ...
                messages = await fake_redis.xreadgroup(
                    "alert-router", "consumer-1", {"alerts": ">"}, count=1
                )
