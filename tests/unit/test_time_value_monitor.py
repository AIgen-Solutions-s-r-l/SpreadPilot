...
from trading_bot.app.service.time_value_monitor import (
    TimeValueMonitor,
    TimeValueStatus,
)
...
def mock_service():
    ...
    service = MagicMock()
    service.active_followers = {
        "test_follower_123": MagicMock(id="test_follower_123")
    }
...
class TestTimeValueMonitor:
    ...
    @pytest.mark.asyncio
    async def test_position_check_with_safe_tv(self, tv_monitor, mock_service, mock_ibkr_client, fake_redis):
        ...
        # Run check
        await tv_monitor._check_follower_positions("test_follower_123", mock_service.active_followers["test_follower_123"])
    ...
    @pytest.mark.asyncio
    async def test_position_check_with_risk_tv(self, tv_monitor, mock_service, mock_ibkr_client, fake_redis):
    ...
    @pytest.mark.asyncio
    async def test_position_liquidation_on_critical_tv(self, tv_monitor, mock_service, mock_ibkr_client, fake_redis):
    ...
    @pytest.mark.asyncio
    async def test_multiple_positions_monitoring(self, tv_monitor, mock_service, mock_ibkr_client, fake_redis):
        ...
        positions = [
            # Safe position
            MagicMock(
                contract=MagicMock(secType="OPT", symbol="QQQ", strike=440.0, right="C"),
                position=5
            ),
            # Risk position
            MagicMock(
        ...
        mock_ibkr_client.get_market_price.side_effect = [
            7.00, 445.0,  # Position 1: option $7, underlying $445, intrinsic $5, TV $2
            5.75, 445.0,  # Position 2: option $5.75, underlying $445, intrinsic $5, TV $0.75
            0.05, 445.0,  # Position 3: option $0.05, underlying $445, intrinsic $0, TV $0.05
        ]
        ...
        assert mock_ibkr_client.get_market_price.call_count == 6  # 2 calls per position (option + underlying)
    ...
    @pytest.mark.asyncio
    async def test_error_handling_market_price_failure(self, tv_monitor, mock_service, mock_ibkr_client, fake_redis):
