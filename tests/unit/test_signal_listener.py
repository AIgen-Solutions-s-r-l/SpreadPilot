...
class TestSignalListener:
    ...
    async def test_signal_listener_initialization(self, signal_listener):
        ...
        assert signal_listener.google_sheet_url == "https://docs.google.com/spreadsheets/test"
    ...
    async def test_start_scheduler(self, signal_listener, mock_redis):
        ...
        with patch('redis.Redis', return_value=mock_redis), \
             patch('trading_bot.app.signal_listener.AsyncIOScheduler') as mock_scheduler_class:
            ...
            mock_scheduler_class.assert_called_once_with(timezone=signal_listener.eastern)
    ...
    @pytest.mark.asyncio
    async def test_gsheet_connection_initialization(self, signal_listener, mock_gsheet_client):
        ...
        with patch('gspread.service_account', return_value=mock_client):
            ...
            mock_client.open_by_url.assert_called_once_with(signal_listener.google_sheet_url)

    @pytest.mark.asyncio
    async def test_gsheet_connection_with_credentials_path(self, signal_listener, mock_gsheet_client):
        """Test Google Sheets connection with credentials path."""
        mock_client, mock_worksheet = mock_gsheet_client
        signal_listener.credentials_path = "/path/to/credentials.json"
        
        with patch('gspread.service_account', return_value=mock_client) as mock_service_account:
            await signal_listener._init_gsheet_connection()
    ...
    @pytest.mark.asyncio
    async def test_poll_for_todays_signal_success(self, signal_listener, mock_gsheet_client):
    ...
    @pytest.mark.asyncio
    async def test_poll_for_todays_signal_timeout(self, signal_listener, mock_gsheet_client):
    ...
    @pytest.mark.asyncio
    async def test_poll_for_todays_signal_progressive_fill(self, signal_listener, mock_gsheet_client):
        ...
        mock_worksheet.get_all_values.side_effect = [sheet_data_empty, sheet_data_filled]
    ...
    @pytest.mark.asyncio
    async def test_fetch_signal_job_success(self, signal_listener, mock_gsheet_client, mock_redis):
