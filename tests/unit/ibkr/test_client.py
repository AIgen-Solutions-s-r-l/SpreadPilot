...
from ib_insync import Stock, Contract, Order, BarData, Trade as IBTrade, OrderStatus as IBOrderStatus, Fill, CommissionReport
...
MOCK_BAR_DATA = [
    BarData(date=datetime.datetime(2025, 4, 18, 10, 0, 0), open=150.0, high=151.0, low=149.0, close=150.5, volume=1000, average=150.2, barCount=100),
    BarData(date=datetime.datetime(2025, 4, 18, 10, 1, 0), open=150.5, high=151.5, low=149.5, close=151.0, volume=1200, average=150.8, barCount=120),
]
...
MOCK_TRADE = IBTrade(
    ...
    fills=[Fill(
        ...
        commissionReport=CommissionReport(commission=1.0, currency='USD'), # Added mock commission report
...
async def test_request_historical_data_success(ibkr_client: IBKRClient, mock_ib_insync: MagicMock):
    ...
    bars = await ibkr_client.request_historical_data(MOCK_STOCK_CONTRACT, durationStr='5 D', barSizeSetting='1 hour')
...
@pytest.mark.asyncio
async def test_request_historical_data_not_connected(ibkr_client: IBKRClient, mock_ib_insync: MagicMock):
...
@pytest.mark.asyncio
async def test_request_historical_data_api_error(ibkr_client: IBKRClient, mock_ib_insync: MagicMock):
...
@pytest.mark.asyncio
async def test_request_stock_positions_case_insensitive(ibkr_client: IBKRClient, mock_ib_insync: MagicMock):
...
@pytest.mark.asyncio
async def test_request_stock_positions_not_connected(ibkr_client: IBKRClient, mock_ib_insync: MagicMock):
...
@pytest.mark.asyncio
async def test_request_stock_positions_api_error(ibkr_client: IBKRClient, mock_ib_insync: MagicMock):
...
@pytest.mark.asyncio
async def test_request_stock_positions_empty_symbols(ibkr_client: IBKRClient, mock_ib_insync: MagicMock):
...
@pytest.mark.asyncio
async def test_request_stock_positions_no_matching(ibkr_client: IBKRClient, mock_ib_insync: MagicMock):
