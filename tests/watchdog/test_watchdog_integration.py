...
def wait_for_alert(redis_client, container_name, alert_type, timeout=60):
    ...
    while time.time() - start_time < timeout:
        ...
        for alert in alerts:
            ...
            if (params.get('container_name') == container_name and 
                alert.get('event_type') == alert_type):
                return alert
        time.sleep(1)
    return None
...
class TestWatchdogIntegration:
    ...
    def test_healthy_service_monitoring(self, docker_client, redis_client):
        ...
        alerts = get_alerts_from_redis(redis_client)
        healthy_alerts = [a for a in alerts if a.get('params', {}).get('container_name') == 'test-healthy-service']
