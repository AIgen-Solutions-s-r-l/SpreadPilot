...
def migrate_secrets(dry_run: bool = False, env_file: Optional[str] = None) -> bool:
    ...
    if not vault_token:
        logger.error("VAULT_TOKEN not found in environment. Please set it to authenticate with Vault.")
        return False
    ...
    for vault_path, secrets in secrets_by_path.items():
        if dry_run:
            ...
            try:
                ...
                # Store to Vault
                success = vault_client.put_secret(
                    vault_path.replace("secret/", ""),
                    existing
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Migrate SpreadPilot secrets to HashiCorp Vault"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--vault-token",
        type=str,
        help="Vault authentication token (default: VAULT_TOKEN env var)"
    )
