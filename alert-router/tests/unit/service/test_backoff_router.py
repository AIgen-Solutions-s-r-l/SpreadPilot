...
class TestBackoffAlertRouter:
    ...
    @pytest.mark.asyncio
    async def test_route_alert_with_backoff_success_first_try(
    ...
    @pytest.mark.asyncio
    async def test_route_alert_with_backoff_retry_then_success(
        self, backoff_router, sample_alert_event
    ):
        ...
        mock_alert_router.route_alert = AsyncMock(
            side_effect=[Exception("Connection error"), {"success": True}]
        )
        backoff_router.alert_router = mock_alert_router
    ...
    @pytest.mark.asyncio
    async def test_route_alert_with_backoff_all_retries_fail(
        self, backoff_router, sample_alert_event
    ):
