...
class BackoffAlertRouter:
    ...
    async def save_alert_attempt(
        ...
        document = {
            ...
            "error": error,
            "results": results,
            "status": "success" if success else ("failed" if attempt >= self.max_retries else "retrying"),
        }
    ...
    async def mark_alert_failed(self, alert_event: AlertEvent, final_error: str):
        ...
        await collection.insert_one(document)
        logger.error(f"Marked alert {alert_event.event_type.value} as permanently failed in MongoDB")
    ...
    async def route_alert_with_backoff(self, alert_event: AlertEvent) -> Dict[str, Any]:
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                logger.info(f"Routing alert {alert_event.event_type.value}, attempt {attempt}/{self.max_retries}")
                ...
                logger.info(f"Successfully routed alert {alert_event.event_type.value} on attempt {attempt}")
                return results
                ...
                if attempt == self.max_retries:
                    ...
                    raise Exception(f"Failed to route alert after {self.max_retries} attempts: {last_error}")
